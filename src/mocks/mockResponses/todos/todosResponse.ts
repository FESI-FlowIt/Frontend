import { Todo } from '@/interfaces/todo';

export const todosRes: { todos: Todo[] } = {
  todos: [
    // UI 테스트용 대량 할일 (30개)
    ...Array.from({ length: 30 }, (_, index) => ({
      todoId: `ui-test-todo-${index + 1}`,
      goalId: 'goal-ui-test',
      title: `UI 테스트 할일 ${index + 1} - ${index < 10 ? '완료된' : '미완료'} 작업입니다`,
      isDone: index < 10, // 처음 10개는 완료, 나머지 20개는 미완료
      attachment: [],
      notes: [],
      createdAt: `2025-07-25T${String(9 + Math.floor(index / 6)).padStart(2, '0')}:${String((index * 10) % 60).padStart(2, '0')}:00.000Z`,
      updatedAt: `2025-07-25T${String(9 + Math.floor(index / 6)).padStart(2, '0')}:${String((index * 10) % 60).padStart(2, '0')}:00.000Z`,
      accumulatedMs:
        index < 10 ? Math.floor(Math.random() * 7200000) : Math.floor(Math.random() * 3600000), // 완료된 것은 최대 2시간, 미완료는 최대 1시간
    })),
    // goal-1 todos
    {
      todoId: 'todo-1',
      goalId: 'goal-1',
      title: '대시보드 UI 구현',
      isDone: true,
      attachment: [],
      notes: [],
      createdAt: '2025-07-10T12:00:00.000Z',
      updatedAt: '2025-07-10T12:00:00.000Z',
      accumulatedMs: 7200000, // 2시간
    },
    {
      todoId: 'todo-2',
      goalId: 'goal-1',
      title: 'API 연동',
      isDone: true,
      attachment: [],
      notes: [],
      createdAt: '2025-07-11T09:00:00.000Z',
      updatedAt: '2025-07-11T09:00:00.000Z',
      accumulatedMs: 5400000, // 1.5시간
    },
    {
      todoId: 'todo-3',
      goalId: 'goal-1',
      title: '캘린더 UI 구현',
      isDone: false,
      attachment: [],
      notes: [],
      createdAt: '2025-07-12T10:00:00.000Z',
      updatedAt: '2025-07-12T10:00:00.000Z',
      accumulatedMs: 1800000, // 30분
    },
    {
      todoId: 'todo-4',
      goalId: 'goal-1',
      title: '할일 UI 구현',
      isDone: false,
      attachment: [],
      notes: [],
      createdAt: '2025-07-13T14:00:00.000Z',
      updatedAt: '2025-07-13T14:00:00.000Z',
      accumulatedMs: 0,
    },
    // goal-2 todos
    {
      todoId: 'todo-5',
      goalId: 'goal-2',
      title: '컴포넌트 리팩토링',
      isDone: true,
      attachment: [],
      notes: [],
      createdAt: '2025-07-18T09:00:00.000Z',
      updatedAt: '2025-07-18T09:00:00.000Z',
      accumulatedMs: 4500000, // 1.25시간
    },
    {
      todoId: 'todo-6',
      goalId: 'goal-2',
      title: '스타일 가이드 적용',
      isDone: true,
      attachment: [],
      notes: [],
      createdAt: '2025-07-19T11:00:00.000Z',
      updatedAt: '2025-07-19T11:00:00.000Z',
      accumulatedMs: 3600000, // 1시간
    },
    {
      todoId: 'todo-7',
      goalId: 'goal-2',
      title: '반응형 웹 구현',
      isDone: true,
      attachment: [],
      notes: [],
      createdAt: '2025-07-20T13:00:00.000Z',
      updatedAt: '2025-07-20T13:00:00.000Z',
      accumulatedMs: 6300000, // 1.75시간
    },
    {
      todoId: 'todo-8',
      goalId: 'goal-2',
      title: '접근성 개선',
      isDone: true,
      attachment: [],
      notes: [],
      createdAt: '2025-07-21T15:00:00.000Z',
      updatedAt: '2025-07-21T15:00:00.000Z',
      accumulatedMs: 2700000, // 45분
    },
    // goal-3 todos
    {
      todoId: 'todo-9',
      goalId: 'goal-3',
      title: '백준 문제 10개 풀기',
      isDone: false,
      attachment: [],
      notes: [],
      createdAt: '2025-07-10T16:00:00.000Z',
      updatedAt: '2025-07-10T16:00:00.000Z',
      accumulatedMs: 2400000, // 40분
    },
    {
      todoId: 'todo-10',
      goalId: 'goal-3',
      title: '정렬 알고리즘 복습',
      isDone: true,
      attachment: [],
      notes: [],
      createdAt: '2025-07-11T18:00:00.000Z',
      updatedAt: '2025-07-11T18:00:00.000Z',
      accumulatedMs: 1800000, // 30분
    },
    {
      todoId: 'todo-11',
      goalId: 'goal-3',
      title: 'DFS/BFS 문제 풀기',
      isDone: false,
      attachment: [],
      notes: [],
      createdAt: '2025-07-12T20:00:00.000Z',
      updatedAt: '2025-07-12T20:00:00.000Z',
      accumulatedMs: 900000, // 15분
    },
    // goal-4 todos
    {
      todoId: 'todo-12',
      goalId: 'goal-4',
      title: '사용자 요구사항 분석',
      isDone: true,
      attachment: [],
      notes: [],
      createdAt: '2025-07-10T14:00:00.000Z',
      updatedAt: '2025-07-10T14:00:00.000Z',
      accumulatedMs: 5400000, // 1.5시간
    },
    {
      todoId: 'todo-13',
      goalId: 'goal-4',
      title: '와이어프레임 작성',
      isDone: false,
      attachment: [],
      notes: [],
      createdAt: '2025-07-11T16:00:00.000Z',
      updatedAt: '2025-07-11T16:00:00.000Z',
      accumulatedMs: 1200000, // 20분
    },
    {
      todoId: 'todo-14',
      goalId: 'goal-4',
      title: '프로토타입 제작',
      isDone: false,
      attachment: [],
      notes: [],
      createdAt: '2025-07-12T17:00:00.000Z',
      updatedAt: '2025-07-12T17:00:00.000Z',
      accumulatedMs: 0,
    },
    {
      todoId: 'todo-15',
      goalId: 'goal-4',
      title: '사용자 테스트 계획',
      isDone: false,
      attachment: [],
      notes: [],
      createdAt: '2025-07-13T18:00:00.000Z',
      updatedAt: '2025-07-13T18:00:00.000Z',
      accumulatedMs: 0,
    },
    // goal-5 todos
    {
      todoId: 'todo-16',
      goalId: 'goal-5',
      title: '주 3회 헬스장 가기',
      isDone: true,
      attachment: [],
      notes: [],
      createdAt: '2025-07-10T19:00:00.000Z',
      updatedAt: '2025-07-10T19:00:00.000Z',
      accumulatedMs: 10800000, // 3시간
    },
    {
      todoId: 'todo-17',
      goalId: 'goal-5',
      title: '매일 30분 러닝',
      isDone: false,
      attachment: [],
      notes: [],
      createdAt: '2025-07-11T19:00:00.000Z',
      updatedAt: '2025-07-11T19:00:00.000Z',
      accumulatedMs: 3600000, // 1시간 (이미 며칠 실행)
    },
    {
      todoId: 'todo-18',
      goalId: 'goal-5',
      title: '스트레칭 루틴 만들기',
      isDone: true,
      attachment: [],
      notes: [],
      createdAt: '2025-07-12T19:30:00.000Z',
      updatedAt: '2025-07-12T19:30:00.000Z',
      accumulatedMs: 1800000, // 30분
    },
    // goal-6 todos
    {
      todoId: 'todo-19',
      goalId: 'goal-6',
      title: 'RESTful API 설계',
      isDone: false,
      attachment: [],
      notes: [],
      createdAt: '2025-07-15T10:00:00.000Z',
      updatedAt: '2025-07-15T10:00:00.000Z',
      accumulatedMs: 2700000, // 45분
    },
    {
      todoId: 'todo-20',
      goalId: 'goal-6',
      title: '인증 시스템 구현',
      isDone: false,
      attachment: [],
      notes: [],
      createdAt: '2025-07-16T11:00:00.000Z',
      updatedAt: '2025-07-16T11:00:00.000Z',
      accumulatedMs: 0,
    },
    {
      todoId: 'todo-21',
      goalId: 'goal-6',
      title: 'API 문서화',
      isDone: false,
      attachment: [],
      notes: [],
      createdAt: '2025-07-17T12:00:00.000Z',
      updatedAt: '2025-07-17T12:00:00.000Z',
      accumulatedMs: 0,
    },
    {
      todoId: 'todo-22',
      goalId: 'goal-6',
      title: '테스트 코드 작성',
      isDone: false,
      attachment: [],
      notes: [],
      createdAt: '2025-07-18T13:00:00.000Z',
      updatedAt: '2025-07-18T13:00:00.000Z',
      accumulatedMs: 0,
    },
    // goal-7 todos
    {
      todoId: 'todo-23',
      goalId: 'goal-7',
      title: 'ERD 작성',
      isDone: true,
      attachment: [],
      notes: [],
      createdAt: '2025-07-12T15:00:00.000Z',
      updatedAt: '2025-07-12T15:00:00.000Z',
      accumulatedMs: 3600000, // 1시간
    },
    {
      todoId: 'todo-24',
      goalId: 'goal-7',
      title: '정규화 작업',
      isDone: true,
      attachment: [],
      notes: [],
      createdAt: '2025-07-13T16:00:00.000Z',
      updatedAt: '2025-07-13T16:00:00.000Z',
      accumulatedMs: 2700000, // 45분
    },
    {
      todoId: 'todo-25',
      goalId: 'goal-7',
      title: '인덱스 설계',
      isDone: false,
      attachment: [],
      notes: [],
      createdAt: '2025-07-14T17:00:00.000Z',
      updatedAt: '2025-07-14T17:00:00.000Z',
      accumulatedMs: 1200000, // 20분
    },
    // goal-8 todos
    {
      todoId: 'todo-26',
      goalId: 'goal-8',
      title: '단위 테스트 작성',
      isDone: true,
      attachment: [],
      notes: [],
      createdAt: '2025-07-08T17:00:00.000Z',
      updatedAt: '2025-07-08T17:00:00.000Z',
      accumulatedMs: 5400000, // 1.5시간
    },
    {
      todoId: 'todo-27',
      goalId: 'goal-8',
      title: '통합 테스트 작성',
      isDone: true,
      attachment: [],
      notes: [],
      createdAt: '2025-07-09T18:00:00.000Z',
      updatedAt: '2025-07-09T18:00:00.000Z',
      accumulatedMs: 7200000, // 2시간
    },
    {
      todoId: 'todo-28',
      goalId: 'goal-8',
      title: 'E2E 테스트 구축',
      isDone: true,
      attachment: [],
      notes: [],
      createdAt: '2025-07-10T19:00:00.000Z',
      updatedAt: '2025-07-10T19:00:00.000Z',
      accumulatedMs: 9000000, // 2.5시간
    },
    {
      todoId: 'todo-29',
      goalId: 'goal-8',
      title: '테스트 커버리지 90% 달성',
      isDone: true,
      attachment: [],
      notes: [],
      createdAt: '2025-07-11T20:00:00.000Z',
      updatedAt: '2025-07-11T20:00:00.000Z',
      accumulatedMs: 3600000, // 1시간
    },
    // goal-9 todos
    {
      todoId: 'todo-30',
      goalId: 'goal-9',
      title: '컬러 팔레트 정의',
      isDone: true,
      attachment: [],
      notes: [],
      createdAt: '2025-07-05T12:00:00.000Z',
      updatedAt: '2025-07-05T12:00:00.000Z',
      accumulatedMs: 1800000, // 30분
    },
    {
      todoId: 'todo-31',
      goalId: 'goal-9',
      title: '타이포그래피 시스템',
      isDone: false,
      attachment: [],
      notes: [],
      createdAt: '2025-07-06T13:00:00.000Z',
      updatedAt: '2025-07-06T13:00:00.000Z',
      accumulatedMs: 2400000, // 40분
    },
    {
      todoId: 'todo-32',
      goalId: 'goal-9',
      title: '컴포넌트 라이브러리 구축',
      isDone: false,
      attachment: [],
      notes: [],
      createdAt: '2025-07-07T14:00:00.000Z',
      updatedAt: '2025-07-07T14:00:00.000Z',
      accumulatedMs: 0,
    },
  ],
};
